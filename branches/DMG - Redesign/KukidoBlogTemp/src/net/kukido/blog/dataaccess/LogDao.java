...
public class LogDao extends Dao implements Iterator
{
	...
	"delete from LOG_ENTRIES where Entry_ID = :Entry_ID"; // 1 Entry_ID
    
    
    private static final String FIND_DATES_POSTED_SQL = 
        "select distinct"
        + " year(Date_Posted) Year"
        + ",month(Date_Posted) Month"
        + ",dayofmonth(Date_Posted) Date"
        + " from LOG_ENTRIES"
        + " order by Date_Posted desc";
    
    private static final String FIND_ENTRY_AFTER_ID_SQL =
        "select min(Entry_ID) from LOG_ENTRIES where Entry_ID > :Entry_ID";
    
    private static final String FIND_ENTRY_BEFORE_ID_SQL =
        "select max(Entry_ID) from LOG_ENTRIES where Entry_ID < :Entry_ID";
    
    private static final String FIND_BY_ENTRY_ID = 
        "select * from LOG_ENTRIES where Entry_ID = :Entry_ID";
    
    private static final String FIND_BY_ENTRY_IDS_SQL_FORMAT = 
        "select * from LOG_ENTRIES where Entry_ID in {0}";
	...
	throws DataAccessException
    {
	    ...
	    try { new LinkDao().create(logEntry); } catch (Exception ignored) {}            
    ...
    public Collection findByEntryIds(Collection entryIds)
        throws DataAccessException
    {
        Connection conn = null;
    ...
    public LogEntry findByEntryId(int entryId)
	throws DataAccessException
    {
        Connection conn = null;
    ...
    public int findEntryAfter(int entryId)
        throws DataAccessException
    {
        Connection conn = null;
        NamedParamStatement statement = null;
        ...
        try
        {
            ...
            results = statement.executeQuery();
            if (!results.next()) { throw new DataAccessException("Can't find entry after ID " + entryId); }
    ...
    public int findEntryBefore(int entryId)
        throws DataAccessException
    {
        Connection conn = null;
        NamedParamStatement statement = null;
        ...
        try
        {
            ...
            statement.setInt("Entry_ID", entryId);
            results = statement.executeQuery();
            if (!results.next()) { throw new DataAccessException("Can't find entry before ID " + entryId); }
    ...
    private Collection populateEntries(ResultSet results)
    ...
    private void populateEntryHeader(ResultSet results, LogEntryHeader header)
        throws SQLException, DataAccessException
    {   
    ...
    private void populateEntryDetails(Collection entries)
        throws SQLException, DataAccessException
    {
    ...
    /**
     * Creates a new LogEntry and populates it with the data from the
     * current row of given ResultSet.
     */
    private LogEntry populateEntry(ResultSet results)
	throws SQLException, DataAccessException
    {
        LogEntry entry = new LogEntry();
        populateEntryHeader(results, entry);
