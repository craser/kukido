<?xml version="1.0" encoding="ISO-8859-1"?>

<xsl:transform version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    xmlns:gpx="http://www.topografix.com/GPX/1/0"
    >
  <xsl:output
      indent="yes"
      method="html"
      omit-xml-declaration="yes"
      media-type="application/xhtml+xml"
      doctype-public="-//W3C//DTD XHTML 1.0 Strict//EN"
      />
      
  <xsl:param name="mapId">map</xsl:param>
        
<xsl:template match="/">
  <xsl:apply-templates select="gpx:gpx" />
</xsl:template>

<xsl:template match="gpx:gpx">
  <script src="http://maps.google.com/maps?file=api&amp;v=2&amp;key=ABQIAAAAOggD5Fz3iK4oyqrD-5a3rxTtbl1hwI1wrVZ-gcFeSdvKcjZNDhTfeymXLgG1x94ojMlumMHhPx5OnA" type="text/javascript"></script>
  <!-- script src="http://maps.google.com/maps?file=api&amp;v=2&amp;key=ABQIAAAAOggD5Fz3iK4oyqrD-5a3rxTFRfqDGOwfXAlOK-54sJyR4NNS5RRcymeccR_BOTGOd_RmVO8QutZgJg" type="text/javascript"script -->


  <script type="text/javascript">
  
  // This variable holds the Google Map JavaScript object.
  <xsl:value-of select="$mapId" /> = 0;

  function buildWayPoint(point, name, desc, elevation)
  {
      var marker = new GMarker(point);
      GEvent.addListener(marker, "click", function() {
          marker.openInfoWindowHtml("&lt;b&gt;" + name + "&lt;/b&gt;&lt;br&gt;" + desc + "&lt;br&gt;");
      });
      return marker;
  }

  function showMap()
  {
      if (GBrowserIsCompatible()) {
          var div = document.getElementById("<xsl:value-of select="$mapId" />");
          var gmap = new GMap2(div);
          gmap.addControl(new GLargeMapControl());
          gmap.addControl(new GMapTypeControl());
          gmap.addControl(new GScaleControl());
          
          // applying trkpt center:
          <xsl:apply-templates select="//gpx:trkpt[1]" mode="center"/>
          <xsl:for-each select="//gpx:trkseg">
            var points = [];
            <xsl:for-each select="gpx:trkpt">
              /* wpt #<xsl:value-of select="position()" /> */ points.push(new GLatLng(<xsl:value-of select="@lat" />,<xsl:value-of select="@lon" />));
              <xsl:if test="(position() mod 100) = 0">
              gmap.addOverlay(new GPolyline(points));
              points = [];
              points.push(new GLatLng(<xsl:value-of select="@lat" />,<xsl:value-of select="@lon" />));
              </xsl:if>
            </xsl:for-each>
            if (points.length > 0) gmap.addOverlay(new GPolyline(points));
          </xsl:for-each>

          <xsl:for-each select="//gpx:wpt">
          gmap.addOverlay(buildWayPoint(new GLatLng(<xsl:value-of select="@lat" />,<xsl:value-of select="@lon" />)
                                       ,'<xsl:value-of select="gpx:name" />'
                                       ,'<xsl:value-of select="gpx:desc" />'
                                       ,'<xsl:value-of select="gpx:ele" />'));
          </xsl:for-each>

          
          GEvent.addListener(gmap, "click", function(marker, point) {
              if (!marker) { gmap.closeInfoWindow(); }
          });
          
          <xsl:value-of select="$mapId" /> = gmap; // Assign the actual Google Map obj. to the global var.
      }
  }
  
  showMap(); // duh.
  </script>
</xsl:template>


<xsl:template match="gpx:trkpt" mode="center">
    gmap.setCenter(new GLatLng(<xsl:value-of select="@lat" />,<xsl:value-of select="@lon" />), 14);
</xsl:template>

<xsl:template match="gpx:trkpt">
  <tr>
    <td><xsl:value-of select="@lat" /></td>
    <td><xsl:value-of select="@lon" /></td>
  </tr>
</xsl:template>

<xsl:template match="gpx:bounds">
    gmap.setCenter(new GLatLng(<xsl:value-of select="string(@minlat + ((@maxlat - @minlat) div 2))" />
                              ,<xsl:value-of select="string(@minlon + ((@maxlon - @minlon) div 2))" />), 14);
</xsl:template>
</xsl:transform>