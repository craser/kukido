    ...
    <script>
      ...
      function bindMap(elementId)
      {
          if (GBrowserIsCompatible()) {
              ...
              var bounds = buildBounds(<nested:write name="track" property="bounds.minLatitude" />, 
              ...
              gmap.setCenter(new GLatLng(<nested:write name="track" property="center.latitude" />,<nested:write name="track" property="center.longitude" />), zoom);
      ...
      window.onload = function() { fitToScreen(); map = bindMap('map'); map.setMapType(G_PHYSICAL_MAP); addMarkers(descriptions, markers); parseAnchor(); renderMapByName(<nested:write name="map" property="fileName" />); };
          ...
          <tiles:put type="string" name="content">
            <table border="0" width="100%">
              ...
              <tr>
                ...
                <%-- Since GPX (and therefore my mapping code) uses metere for elevation, convert to feet. --%>
            ...
            <tiles:put type="string" name="content">
              <ul class="gallerymenu">
                <nested:iterate name="entry" property="attachments" id="image" type="net.kukido.blog.datamodel.Attachment">
                  <logic:notEmpty name="image" property="geotags">
                    <li><a href="javascript:showImageOnMap('<%= image.getFileName() %>')" title="<%= image.getTitle() %>">
                    <img alt="<%= image.getTitle() %>" class="thumbnail" src="attachments/thumbs/<%= image.getFileName() %>" />
                    </a></li>
                  </logic:notEmpty>
                </nested:iterate>
              </ul>
