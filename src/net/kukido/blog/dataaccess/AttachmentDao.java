...
public class AttachmentDao extends Dao
{
	...
	"select"
        ...
        + ",Description"
        + ",Date_Taken"
        + " from ATTACHMENTS where File_Type = :File_Type";
	...
	"select"
        ...
        + ",Description"
        + ",Date_Taken"
        + " from ATTACHMENTS where File_Name = :File_Name";
    ...
    static private final String FIND_BYTES_BY_ATTACHMENT_ID_SQL = 
        "select bytes from ATTACHMENTS where Attachment_ID = :Attachment_ID";

    static private final String FIND_RECENT_IMAGES_SQL =
        "select * from ATTACHMENTS where Is_Gallery_Image = 'true'"
	...
	{
	    throw new DataAccessException("Unable to delete attachment with ID " + attachmentId, e);
	}
	finally
	{
        ...
        throws DataAccessException
    {
        ...
        {
            throw new DataAccessException("Unable to delete attachments with Entry_ID " + entryId, e);
        }
        finally
        {
            try { attachments.close(); } catch (Exception ignored) {}
	...
	try
	{
	    ...
	    create.setString("Is_Gallery_Image", Boolean.toString(attachment.getIsGalleryImage()));
	...
	{
	    throw new DataAccessException("Unable to create attachment with name " + attachment.getFileName(), e);	    
	}
	finally
	{
    ...
    public void updateAll(Collection attachments)
	throws DataAccessException
    {
	...
	try
	{
	    ...
	    update.setString("Is_Gallery_Image", Boolean.toString(attachment.getIsGalleryImage()));
	    ...
	    update.setDate("Date_Posted", new java.sql.Date(attachment.getDatePosted().getTime()));
	...
	{
	    throw new DataAccessException("Unable to update attachment with name " + attachment.getFileName(), e);	    
	}
	finally
	{
        ...
        throws DataAccessException
    {
        ...
        {
            conn = getConnection();
            find = new NamedParamStatement(conn, FIND_BYTES_BY_ATTACHMENT_ID_SQL);
            find.setInt("Attachment_ID", attachment.getAttachmentId());
            results = find.executeQuery();
            
            if (!results.next()) {
                throw new DataAccessException("Unable to retrieve bytes for attachment ID " + attachment.getAttachmentId());
            }
            ...
            throw new DataAccessException("Unable to retrieve bytes for attacment ID " + attachment.getAttachmentId() + ": " + e.toString(), e);
        }
	finally
	{
    ...
    public Collection findUnattached()
	throws DataAccessException
    {
	...
	{
	    throw new DataAccessException("Unable to find attachments for entry " + entryId, e);
	}
	finally
	{
	    try { results.close(); } catch (Exception ignored) {}
	...
	{
	    throw new DataAccessException("Unable to find attachments for entryIds " + entryIds, e);
	}
	finally
	{
	    try { results.close(); } catch (Exception ignored) {}
	...
	try
	{
	    ...
	    if (!results.next()) throw new DataAccessException("Unable to find attachment with id " + attachmentId);
	...
	{
	    throw new DataAccessException("Unable to find attachment with id " + attachmentId, e);
	}
	finally
	{
	    try { results.close(); } catch (Exception ignored) {}
	...
	{
	    throw new DataAccessException("Unable to find attachment with type \"" + fileType + "\"", e);
	}
	finally
	{
	    try { results.close(); } catch (Exception ignored) {}
	...
	{
	    throw new DataAccessException("Unable to find attachment with name " + fileName, e);
	}
	finally
	{
	    try { results.close(); } catch (Exception ignored) {}
	...
	try
	{
	    ...
	    if (!results.next()) throw new DataAccessException("Unable to find attachment with name " + fileName);
	...
	{
	    throw new DataAccessException("Unable to find " + numEntries + " recent attachments:", e);
	}
	finally
	{
	    try { results.close(); } catch (Exception ignored) {}
	    try { findRecent.close(); } catch (Exception ignored) {}
	    try { conn.close(); } catch (Exception ignored) {}
	}
    }
	...
	Attachment attachment = new Attachment();
        ...
        Collection geotags = new GeotagDao().findByAttachmentId(attachment.getAttachmentId());
        attachment.setGeotags(geotags);
        return attachment;
    }
    
    private void populateAttachmentHeader(ResultSet results, Attachment attachment)
        throws SQLException, DataAccessException
    {
	...
	attachment.setDescription(results.getString("Description"));
        ...
        // as images.
        if (attachment.getIsGalleryImage() && (attachment.getDateTaken() == null)) {
            try
            {
		...
		System.out.println("Getting date taken for \"" + attachment.getFileName() + "\"");
                java.util.Date dateTaken = tools.getDateTaken(bytes);
		System.out.println("Done getting date taken for \"" + attachment.getFileName() + "\"");
                
                if (dateTaken == null) { throw new NullPointerException("No Date Taken Found!"); }
